...
function runtimeScript(): string {
  return `(function(){const call=async(method,body)=>{const response=await fetch("${meAPIPath}",{method,headers:{"content-type":"application/json"},body:body===undefined?undefined:JSON.stringify(body)});if(!response.ok)throw new Error("Intern local ME request failed with HTTP "+response.status);return response.json()};const encode=(bytes)=>{let binary="";for(let offset=0;offset<bytes.length;offset+=32768)binary+=String.fromCharCode(...bytes.subarray(offset,offset+32768));return btoa(binary)};const me=Object.freeze({get:()=>call("GET"),update:(input)=>call("PATCH",{...input,profilePicture:input.profilePicture&&{data:encode(input.profilePicture.bytes),contentType:input.profilePicture.contentType,filename:input.profilePicture.filename}})});const runtime=Object.freeze({protocolVersion:${INTERN_PROTOCOL_VERSION},plugins:Object.freeze({me})});globalThis.intern=Object.freeze({protocolVersion:${INTERN_PROTOCOL_VERSION},resolveRuntime:()=>runtime})})();`;
}
